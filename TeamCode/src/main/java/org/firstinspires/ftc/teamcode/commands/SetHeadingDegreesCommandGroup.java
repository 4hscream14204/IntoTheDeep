...
public class SetHeadingDegreesCommandGroup extends SequentialCommandGroup {
    public SetHeadingDegreesCommandGroup(RobotBase robotBase, double targetHeadingDegrees){
        if(Math.abs(Math.toDegrees(robotBase.drive.getPose().getHeading()) - targetHeadingDegrees) >= 170){
            addCommands(
                    new InstantCommand(()->robotBase.chassisSubsystem.setTargetDegrees(Math.toDegrees(robotBase.drive.getPose().getHeading() - 90))),
                    new WaitCommand(100),
